export const data = [
  {
    name: 'Eoin',
    teams: ['GER', 'CRO', 'KOR', 'QAT'],
    image: 'college/eoin.jpg',
  },
  {
    name: 'Peter',
    teams: ['ENG', 'CRO', 'ECU', 'CMR'],
    image: 'pintmen/peter.jpg',
  },
  {
    name: 'Austin',
    teams: ['POR', 'BEL', 'POL', 'GHA'],
    image: 'pintmen/austin.jpg',
  },
  {
    name: 'Sean',
    teams: ['NED', 'WAL', 'SRB', 'CAN'],
    image: 'pintmen/sean.jpg',
  },
  {
    name: 'Mark',
    teams: ['ESP', 'URU', 'AUS', 'KSA'],
    image: 'pintmen/mark.jpg',
  },
  {
    name: 'Dale',
    teams: ['ARG', 'DEN', 'JPN', 'IRN'],
    image: 'pintmen/dale.jpg',
  },
  {
    name: 'Martin',
    teams: ['GER', 'SUI', 'MAR', 'TUN'],
    image: 'pintmen/martin.jpg',
  },
  {
    name: 'Serena',
    teams: ['FRA', 'SEN', 'USA', 'CRC'],
    image: 'pintmen/serena.jpg',
  },
  {
    name: 'Waheed',
    teams: ['BRA', 'MEX', 'JPN', 'GHA'],
    image: 'pintmen/waheed.jpg',
  },
];
